...
export const createLeadActivity = async (leadId, data) => {
  const res = await API.post(`/activities/${leadId}`, data);
  return res.data;
};
...
export const getSmartQueue = async () => {
  try {
    const res = await API.get("/leads/smart-queue");
    return res.data;
  } catch (error) {
    if (error?.response?.status === 401 || error?.response?.status === 403) {
      throw new Error("Session expired. Please login again.");
    ...
    throw new Error(
      error?.response?.data?.detail ||
        error?.response?.data?.error ||
        error?.message ||
        "Unable to load smart queue",
    );
  }
};

export const getLeadResearch = async (leadId) => {
  try {
    const res = await API.get(`/leads/research/${leadId}`);
    return res.data;
  } catch (error) {
    if (error?.response?.status === 401 || error?.response?.status === 403) {
      throw new Error("Session expired. Please login again.");
    ...
    throw new Error(
      error?.response?.data?.detail ||
        error?.response?.data?.error ||
        error?.message ||
        "Unable to generate AI lead research",
    );
  }
};

export const generateMeetingNotes = async (leadId, notes) => {
  try {
    const res = await API.post(`/leads/meeting-notes/${leadId}`, { notes });
    return res.data;
  } catch (error) {
    if (error?.response?.status === 401 || error?.response?.status === 403) {
      throw new Error("Session expired. Please login again.");
    ...
    throw new Error(
      error?.response?.data?.detail ||
        error?.response?.data?.error ||
        error?.message ||
        "Unable to generate AI meeting notes",
    );
  }
};

export const getDealRisk = async (leadId) => {
  const res = await API.get(`/leads/deal-risk/${leadId}`);
  return res.data;
};

export const getHealthScore = async (leadId)=>{
 const res = await API.get(`/leads/health-score/${leadId}`);
 return res.data;
};
